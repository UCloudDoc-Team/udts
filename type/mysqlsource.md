...
- 全量迁移阶段， 空数据库不迁移。
- 不迁移除 test 以外的内置数据库。
- 在转储过程中，请勿执行库或表结构变更的DDL操作，否则DDL操作会被阻塞到转储完成。
...
| 源最大速率    | 外网/专线的速率范围为 1-56 MB/s，默认28 MBps (即 224 Mbps); 内网的速率范围为 1-1024 MB/s，默认64 MBps(即512Mbps)         |
| 目标最大速率    | 内网的速率范围为 1-1024 MB/s，默认64 MBps(即512Mbps)         |
