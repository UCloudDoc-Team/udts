...
2. 不支持 delete 语句。支持迁移 ddl 语句，详见“迁移内容--DDL”。
...
5. 选择“手动创建库表”时，UDTS 将不会在目标库创建库表，请您在目标库中提前创建待迁移库表，再启动迁移任务。增量任务选择“手动创建库表”时，不会同步 DDL 语句。
6. 选择“自动创建库表”时，如果待迁移的库表在目标库中不存在，UDTS 将在目标库中自动创建待迁移的库表；如果待迁移的库表在目标库中已经存在，UDTS 不会重新创建表结构。增量任务选择“自动创建库表”时，会自动同步增量运行期间产生的同步 DDL 语句。
