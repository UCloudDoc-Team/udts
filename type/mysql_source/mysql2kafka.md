...
| Topic前缀    | UDTS迁移MySQL到Kafka会在目标Kafka上创建对应的Topic， 规则是每张表创建一个Topic，每个Topic都会以此参数作为前缀(格式为: Topic前缀.数据库名.表名) |
...
UDTS 会为每一张表创建一个Topic，没张表的数据会写入到其对应的Topic中，Topic的名称为: Topic前缀.数据库名.表名。