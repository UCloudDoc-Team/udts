...
当您的UDTS迁移任务成功将数据同步至目标库后，可参考本文进行业务切换，以最大程度减少迁移对业务的影响。本文以迁移实例为例，介绍业务切换的相关步骤。

## 前提条件

- 已创建并配置好UDTS迁移任务，且任务状态为运行中或已完成。
- 配置方法请参见迁移方案概览。
...
4. **暂停迁移任务。**
   ...
   - 等待迁移任务状态变为"已同步"，或者剩余数据量为0。
     > **说明**  
     > 任务状态为“已同步”或剩余数据量为0可能是短暂状态，此时源库待迁移数据已成功迁移至目标库。
   - 继续等待1分钟，观察任务状态，保持为“已同步”或者剩余数据量0且不再变化。
...
业务切换至目标库并稳定运行一段时间，测试所有业务涉及的功能并确认无问题后，可结束反向数据迁移任务。