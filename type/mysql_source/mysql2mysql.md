...
UDTS支持MySQL作为数据传输源/目标，支持版本有 MySQL(包含Percona版)5.5/5.6/5.7/8.0，以及兼容MySQL的数据库， 像PolarDB等。
...
UDTS任务在迁移过程中会在目标库中创建 `syncer_meta_schema`，目标库账号需要保证具有此库的所有权限。迁移过程中请不要删除`syncer_meta_schema`，否则会导致迁移任务失败。
...
建议单个全量迁移任务所迁移的数据量不超过200G， 最大支持1000G。 如果所要迁移的数据量超过了1000G，可以将任务拆分为多个任务进行迁移。 UDTS 提供按库、按表、按多库、按多表等多维度的迁移方式。 

如果迁移任务超过了1000G， 且无法拆分为多个任务， 请联系技术支持。
